...
const { VITE_APP_USERNAME_SUPER_ADMIN,
        VITE_APP_EMAIL_SUPER_ADMIN, 
        VITE_APP_PASSWORD_SUPER_ADMIN, 
        VITE_APP_TYPE_SUPER_ADMIN,
        VITE_APP_LEVEL_SUPER_ADMIN,
        VITE_APP_API,
         } = import.meta.env;
...
const validUser = async()=> 
{
  ...
  if (email === VITE_APP_EMAIL_SUPER_ADMIN) {
    if (password === VITE_APP_PASSWORD_SUPER_ADMIN) {
      ...
      try {
          const responseToken = await axios.get(`${VITE_APP_API}/user/login/gettoken/${email}`);
          if (responseToken) {
            const data = {
              userName  : VITE_APP_USERNAME_SUPER_ADMIN, 
              email     : VITE_APP_EMAIL_SUPER_ADMIN,
              typeUser  : VITE_APP_TYPE_SUPER_ADMIN,
              levelUser : VITE_APP_LEVEL_SUPER_ADMIN,
    ...
    //passworr was correct
      ...
      {
        try {
          const response = await axios.get(`${VITE_APP_API}/user/login/${email}`);
          const { data } = response;          
